# Click run to run the model. If there is a need to change the sample size or K value, edit the last call to
# precision_and_diversity_at_k function to whatever the requirements are
...
# Load the dataset
df = pd.read_csv('../../Datasets/CBF/base_information.csv')
popularity_df = pd.read_csv('../../Datasets/CBF/CBF_Additional_Features.csv')
...
#Prepares the data to numerical format for use. This involves MinMaxScaler, OneHotEncoder and TF-IDF
scaler = MinMaxScaler()
tfidf_vectorizer = TfidfVectorizer(stop_words='english')
popularity = popularity_df['owners'].apply(lambda x: get_owner_range(int(x.split("..")[1].replace(',', '').strip())))
novelty_scores = scaler.fit_transform(popularity.values.reshape(-1, 1))
popularity_df['novelty_score'] = novelty_scores
id_to_novelty_score = dict(zip(popularity_df['id'], popularity_df['novelty_score']))
novelty_scores = [id_to_novelty_score.get(id_value, 0) for id_value in df['id']]
df['novelty_score'] = novelty_scores
df2 = df.drop(columns=['id', 'app_name'])
...
categorical_cols = ['publisher', 'developer', 'early_access', 'sentiment', 'release_date']
...
def get_user_recommendations(user_games, K=10):
    # Compute Aggregate Feature Vector
